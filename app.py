...
with tabs[3]:
    ...
    if has_odds_key:
        ...
        bk_choice = ob2.selectbox(
            "Bookmaker",
            options=list(BOOKMAKER_LABELS.keys()),
            format_func=lambda x: BOOKMAKER_LABELS[x],
            index=0, key="bk_sel"
        )
        if ob1.button("🔄 Fetch odds from API", type="secondary", use_container_width=True):
            ...
            if api_fixtures:
                ...
                ob3.success(f"✓ {len(api_fixtures)} fixtures loaded  |  {rem} API calls remaining")
            else:
                ob3.warning("No upcoming fixtures found — may not be NRL week")
    else:
        st.markdown('<div class="info">Add <code>ODDS_API_KEY = "your_key"</code> to Streamlit Secrets to auto-fill odds. Free key at <a href="https://the-odds-api.com" target="_blank">the-odds-api.com</a></div>', unsafe_allow_html=True)
