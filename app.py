...
from xladder_pipeline import (
    ...
    parse_gerard_round, parse_gerard_odds, GERARD_STATS_M4,
    aggregate_player_to_team, build_halfline_table,
    get_top_players, get_form_players, PLAYER_DISPLAY,
)
...
def read_master_from_sheets(league):
    ...
    try:
        ...
        for col in df.columns:
            ...
            if col in ["Season","Round","Match ID","A Team","B Team","Home Advantage",
                       "League","A_Name","B_Name","Venue","Date","Kick-off (local)"]:
                continue
            # Try 1: direct numeric conversion
            converted = pd.to_numeric(col_data, errors="coerce")
            # Try 2: comma as decimal separator (e.g. "4,41" → "4.41")
            if converted.isna().mean() > 0.5:
                col_fixed = col_data.str.replace(",", ".", regex=False)
                converted2 = pd.to_numeric(col_fixed, errors="coerce")
                if converted2.notna().sum() > converted.notna().sum():
                    converted = converted2
            # Try 3: thousands separator (e.g. "8,296,476" → "8296476")
            if converted.isna().mean() > 0.5:
                col_fixed2 = col_data.str.replace(",", "", regex=False)
                converted3 = pd.to_numeric(col_fixed2, errors="coerce")
                if converted3.notna().sum() > converted.notna().sum():
                    converted = converted3
            # Only apply if meaningful improvement
            orig_notna = (col_data != "nan").sum()
            if converted.notna().sum() >= max(orig_notna * 0.3, 1):
                df[col] = converted
        return df, True
...
NRL_API_NAMES = {
    ...
    # All others match master names exactly
}
SL_API_NAMES = {
    "Wigan Warriors":"WI","St Helens":"SH","Leeds Rhinos":"LH",
    "Catalans Dragons":"CF","Warrington Wolves":"WFT","Hull KR":"HKR",
    "Leigh Leopards":"LS","Castleford Tigers":"C","Huddersfield Giants":"HFC",
    "Wakefield Trinity":"WA","Toulouse Olympique":"TL","Hull FC":"HF",
    "Bradford Bulls":"BD","York Knights":"YK","York RLFC Knights":"YK",
}

BOOKMAKER_LABELS = {
    "sportsbet":"Sportsbet","tab":"TAB","ladbrokes_au":"Ladbrokes",
    "neds":"Neds","pointsbetau":"PointsBet","betright":"BetRight",
    "unibet":"Unibet","betr_au":"Betr","tabtouch":"TABtouch","playup":"PlayUp",
}

def fetch_odds(league: str, bookmaker: str = "sportsbet",
               days_ahead: int = 7) -> list:
    """
    Fetch upcoming fixtures + odds from The Odds API.
    Groups all games within the next `days_ahead` days as the upcoming round.
    Returns list of fixture dicts ready for the Betting tab.
    """
    ...
    sport  = "rugbyleague_nrl" if league=="NRL" else "rugbyleague_nrl"  # SL not on API yet
    region = "au"
    ...
    if isinstance(data, dict) and "message" in data:
        st.warning(f"Odds API: {data['message']}")
        return []

    # Group by week — find earliest date, take all games within 7 days of that
    if not data:
        return []
    dates = [g["commence_time"][:10] for g in data]
    earliest = min(dates)
    cutoff_dt = datetime.fromisoformat(earliest) + timedelta(days=days_ahead)

    name_fix = NRL_API_NAMES if league=="NRL" else {}
    nm = NRL_SHORT if league=="NRL" else SL_NAMES
    rev = {v:k for k,v in nm.items()}

    results = []
    for g in data:
        game_date = g["commence_time"][:10]
        if datetime.fromisoformat(game_date) > cutoff_dt:
            continue

        ht_api = g.get("home_team",""); at_api = g.get("away_team","")
        # Apply name fixes
        ht_model = name_fix.get(ht_api, ht_api)
        at_model = name_fix.get(at_api, at_api)
        # For SL use code mapping
        if league=="SL":
            ht_model = SL_API_NAMES.get(ht_api, ht_api)
            at_model = SL_API_NAMES.get(at_api, at_api)
        ht_disp = nm.get(ht_model, ht_model)
        at_disp = nm.get(at_model, at_model)

        # Parse markets from chosen bookmaker
        h2h_h=1.91; h2h_a=1.91; line=0.0; lo_h=1.91; lo_a=1.91
        total=45.5; tot_odds=1.91
        for bk in g.get("bookmakers",[]):
            if bk["key"] != bookmaker:
                continue
            for mkt in bk.get("markets",[]):
                k = mkt["key"]; outs = mkt.get("outcomes",[])
                if k=="h2h":
                    for o in outs:
                        if o["name"]==ht_api: h2h_h=o["price"]
                        elif o["name"]==at_api: h2h_a=o["price"]
                elif k=="spreads":
                    for o in outs:
                        if o["name"]==ht_api: line=o.get("point",0); lo_h=o["price"]
                        elif o["name"]==at_api: lo_a=o["price"]
                elif k=="totals":
                    for o in outs:
                        if o["name"]=="Over": total=o.get("point",45.5); tot_odds=o["price"]
            break

        results.append({
            "hd":ht_disp, "ad":at_disp,
            "hc":ht_model, "ac":at_model,
            "ha":"A",
            "h2h_h":round(h2h_h,3), "h2h_a":round(h2h_a,3),
            "line":float(line), "lo_h":round(lo_h,3), "lo_a":round(lo_a,3),
            "total_line":float(total), "tot_odds":round(tot_odds,3),
            "date": game_date,
        })
    return results
...
# ── Gerard extended data session state keys ──────────────────────────────────
if "gerard_player_df"  not in st.session_state: st.session_state["gerard_player_df"]  = None
if "gerard_tryscore_df" not in st.session_state: st.session_state["gerard_tryscore_df"] = None
if "gerard_restart_df" not in st.session_state: st.session_state["gerard_restart_df"]  = None
...
@st.cache_data(show_spinner="Loading data...", ttl=30)
...
# Force reload if coming from a submit
...
def predict_fx(hc,ac,ha):
    ...
    for fc in form_cols:
        fh=team_form_m.get((hc,fc),0) or 0
        fa=team_form_m.get((ac,fc),0) or 0
        try: fh=float(fh) if not (fh!=fh) else 0.0  # nan check
        except: fh=0.0
        try: fa=float(fa) if not (fa!=fa) else 0.0
        except: fa=0.0
        feat[fc]=fh-fa
    X=np.array([[feat.get(f,0) for f in F_m]],dtype=float)
...
tab_names=["📊 Dashboard","🏆 xLadder","📈 Team Stats","🎯 Betting","📒 Bet History","🔬 Model","📥 Weekly Input","🏃 Players"]
...
with tabs[1]:
    ...
    with c2:
        ...
        # Diverging bar: sorted by PPG_Diff, best at top
        xl_s = xl.sort_values("PPG_Diff", ascending=True).reset_index(drop=True)
        colors = ["#10B981" if d>=0 else "#EF4444" for d in xl_s["PPG_Diff"]]
        fig=go.Figure(go.Bar(
            y=xl_s["Team"], x=xl_s["PPG_Diff"], orientation="h",
            marker_color=colors, opacity=0.88,
            text=[f"{v:+.2f}" for v in xl_s["PPG_Diff"]],
            textposition="outside",
            textfont=dict(size=11, color="#F1F5F9"),
            hovertemplate="<b>%{y}</b><br>xPPG: %{customdata[0]:.3f}<br>Actual: %{customdata[1]:.3f}<br>Diff: %{x:+.3f}<extra></extra>",
            customdata=list(zip(xl_s["Expected_PPG"], xl_s["Actual_PPG"]))
        ))
        fig.add_vline(x=0, line_dash="dash", line_color="#64748B", line_width=1.5)
        # Add xPPG reference as subtle text
        for _,row in xl_s.iterrows():
            fig.add_annotation(
                x=0, y=row["Team"],
                text=f"xPPG {row['Expected_PPG']:.2f}",
                showarrow=False,
                font=dict(size=9, color="#475569"),
                xanchor="left" if row["PPG_Diff"]<0 else "right",
                xshift=8 if row["PPG_Diff"]<0 else -8
            )
        at(fig, height=520,
            xaxis=dict(title="Actual PPG − Expected PPG (model)", gridcolor="#334155",
                       zeroline=False, tickformat="+.2f"),
            yaxis=dict(gridcolor="#334155"),
            title_text="Who is over/underperforming the model?",
            legend=dict(orientation="h",yanchor="bottom",y=1.02,xanchor="right",x=1))
        st.plotly_chart(fig,use_container_width=True)
        st.markdown('<div class="info"><b>Green (+)</b> = winning more than model expects — may regress. &nbsp; <b>Red (−)</b> = underperforming model expectations.</div>', unsafe_allow_html=True)
...
with tabs[3]:
    ...
    # ── Auto-fill from Odds API ─────────────────────────────────────────────
    # ── Live Odds from API ──────────────────────────────────────────────────────
    try:
        _odds_key = st.secrets["ODDS_API_KEY"]
        has_odds_key = bool(_odds_key)
    except Exception as _e:
        has_odds_key = False
        _odds_key = None

    # Debug: show secrets keys (remove after fixing)
    with st.expander("🔧 Debug — Secrets check", expanded=not has_odds_key):
        try:
            _all_keys = list(st.secrets.keys())
            st.write("Secret keys found:", _all_keys)
            st.write("ODDS_API_KEY present:", "ODDS_API_KEY" in _all_keys)
            if has_odds_key:
                st.write("ODDS_API_KEY value:", f"...{str(_odds_key)[-6:]}")
        except Exception as _de:
            st.write("Error reading secrets:", str(_de))

    if has_odds_key and league == "NRL":
        ob1, ob2, ob3 = st.columns([2, 2, 3])
        bk_choice = ob2.selectbox("Bookmaker",
            options=list(BOOKMAKER_LABELS.keys()),
            format_func=lambda x: BOOKMAKER_LABELS[x],
            index=0, key="bk_sel")
        fetch_clicked = ob1.button("🔄 Fetch live odds", type="primary", use_container_width=True)

        if fetch_clicked:
            with st.spinner(f"Fetching {BOOKMAKER_LABELS[bk_choice]} odds..."):
                api_fixtures = fetch_odds(league, bookmaker=bk_choice)
            if api_fixtures:
                st.session_state["api_fixtures"] = api_fixtures
                rem = st.session_state.get("odds_api_remaining", "?")
                ob3.success(f"✓ {len(api_fixtures)} fixtures  |  {rem} calls remaining")
                # Pre-populate ALL widget session states so inputs show correct values
                for _i, _fx in enumerate(api_fixtures):
                    _hd_idx = disp_teams.index(_fx["hd"]) if _fx["hd"] in disp_teams else 0
                    _ad_idx = disp_teams.index(_fx["ad"]) if _fx["ad"] in disp_teams else 1
                    st.session_state[f"bh_{_i}"]    = _hd_idx
                    st.session_state[f"ba_{_i}"]    = _ad_idx
                    st.session_state[f"h2hh_{_i}"]  = float(_fx["h2h_h"])
                    st.session_state[f"h2ha_{_i}"]  = float(_fx["h2h_a"])
                    st.session_state[f"line_{_i}"]  = float(_fx["line"])
                    st.session_state[f"loh_{_i}"]   = float(_fx["lo_h"])
                    st.session_state[f"loa_{_i}"]   = float(_fx["lo_a"])
                    st.session_state[f"tot_{_i}"]   = float(_fx["total_line"])
                    st.session_state[f"todo_{_i}"]  = float(_fx["tot_odds"])
                st.session_state["bet_nf"] = len(api_fixtures)
                st.rerun()
            else:
                ob3.warning("No upcoming fixtures found")

        # Always show live odds table if available
        api_fx_show = st.session_state.get("api_fixtures", [])
        if api_fx_show:
            st.markdown('<div class="shr">Live odds — use these to fill fixtures below</div>', unsafe_allow_html=True)
            odds_rows = []
            for fx in api_fx_show:
                odds_rows.append({
                    "Home": fx["hd"], "Away": fx["ad"],
                    "H2H H": fx["h2h_h"], "H2H A": fx["h2h_a"],
                    "Line": fx["line"],
                    "Line H": fx["lo_h"], "Line A": fx["lo_a"],
                    "Total": fx["total_line"], "Tot Odds": fx["tot_odds"],
                    "Date": fx.get("date",""),
                })
            odds_df = pd.DataFrame(odds_rows)
            st.dataframe(odds_df, use_container_width=True, height=320)
            st.markdown('<div class="info">These are the upcoming fixtures from the bookmaker. Enter matching fixtures in the form below and the odds will auto-fill.</div>', unsafe_allow_html=True)
    elif league == "SL":
        st.markdown('<div class="info">Live odds auto-fill is available for NRL only. Enter SL lines manually below.</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="info">Add <code>ODDS_API_KEY = "your_key"</code> to Streamlit Secrets for live odds. Free key at <a href="https://the-odds-api.com" target="_blank">the-odds-api.com</a></div>', unsafe_allow_html=True)

    # ── Gerard odds CSV upload (works without API key) ───────────────────────
    with st.expander("📂 Upload odds from Gerard CSV file"):
        gerard_odds_file = st.file_uploader("Upload NRL odds CSV (Gerard format)",
                                             type=["csv"], key="gerard_odds")
        if gerard_odds_file:
            try:
                odds_df = pd.read_csv(gerard_odds_file)
                gerard_fx = parse_gerard_odds(odds_df)
                if gerard_fx:
                    st.session_state["api_fixtures"] = gerard_fx
                    # Pre-populate widget states
                    for _i,_fx in enumerate(gerard_fx):
                        _hdi = disp_teams.index(_fx["hd"]) if _fx["hd"] in disp_teams else 0
                        _adi = disp_teams.index(_fx["ad"]) if _fx["ad"] in disp_teams else 1
                        st.session_state[f"bh_{_i}"]   = _hdi
                        st.session_state[f"ba_{_i}"]   = _adi
                        st.session_state[f"h2hh_{_i}"] = float(_fx["h2h_h"])
                        st.session_state[f"h2ha_{_i}"] = float(_fx["h2h_a"])
                        st.session_state[f"line_{_i}"] = float(_fx["line"])
                        st.session_state[f"loh_{_i}"]  = float(_fx["lo_h"])
                        ...
                        st.session_state[f"tot_{_i}"]  = float(_fx["total_line"])
                        st.session_state[f"todo_{_i}"] = float(_fx["tot_odds"])
                    st.session_state["bet_nf"] = len(gerard_fx)
                    st.success(f"✓ {len(gerard_fx)} fixtures loaded from Gerard odds file")
                    st.rerun()
            except Exception as e:
                st.error(f"Odds parse error: {e}")

    # Use API fixtures if available, otherwise manual entry
    api_fx = st.session_state.get("api_fixtures", [])
    n_fix_default = len(api_fx) if api_fx else (8 if league=="NRL" else 6)
    n_fix=st.number_input("Fixtures",1,16,n_fix_default,key="bet_nf")
    fixtures=[]
    for i in range(int(n_fix)):
        with st.expander(f"Fixture {i+1}",expanded=(i<4)):
            # Pre-fill from API if available
            _af = api_fx[i] if i < len(api_fx) else {}
            _hd_def = _af.get("hd", disp_teams[min(i*2,   len(disp_teams)-1)])
            _ad_def = _af.get("ad", disp_teams[min(i*2+1, len(disp_teams)-1)])
            _hd_idx = disp_teams.index(_hd_def) if _hd_def in disp_teams else min(i*2,   len(disp_teams)-1)
            _ad_idx = disp_teams.index(_ad_def) if _ad_def in disp_teams else min(i*2+1, len(disp_teams)-1)
            fc1,fc2,fc3=st.columns([2,2,1])
            hd=fc1.selectbox("Home",disp_teams,key=f"bh_{i}",index=_hd_idx)
            ad=fc2.selectbox("Away",disp_teams,key=f"ba_{i}",index=_ad_idx)
            ...
            h2h_h=oc1.number_input("H2H Home",1.01,20.0,float(_af.get("h2h_h",1.80)),0.05,key=f"h2hh_{i}",format="%.2f")
            h2h_a=oc2.number_input("H2H Away",1.01,20.0,float(_af.get("h2h_a",2.05)),0.05,key=f"h2ha_{i}",format="%.2f")
            line=oc3.number_input("Line",-50.0,50.0,float(_af.get("line",-6.5)),0.5,key=f"line_{i}",format="%.1f")
            lo_h=oc4.number_input("Line H",1.01,5.0,float(_af.get("lo_h",1.91)),0.01,key=f"loh_{i}",format="%.2f")
            lo_a=oc5.number_input("Line A",1.01,5.0,float(_af.get("lo_a",1.91)),0.01,key=f"loa_{i}",format="%.2f")
            # Total over/under
            oc6,oc7=st.columns(2)
            total_line=oc6.number_input("Total O/U",20.0,100.0,float(_af.get("total_line",45.5)),0.5,key=f"tot_{i}",format="%.1f")
            tot_odds=oc7.number_input("Total odds",1.01,5.0,float(_af.get("tot_odds",1.91)),0.01,key=f"todo_{i}",format="%.2f")
    ...
    if st.button("Generate predictions & edges",type="primary",use_container_width=True):
        ...
        # 1H/2H context if tryscorer data available
        trysco_ss = st.session_state.get("gerard_tryscore_df")
        if trysco_ss is not None:
            try:
                half_t = build_halfline_table(trysco_ss)
                if len(half_t):
                    team_half = half_t.groupby("team")[["1H","2H","total"]].sum().reset_index()
                    team_half["1H%"] = (team_half["1H"]/team_half["total"]*100).round(0)
                    st.markdown('<div class="shr">1H scoring tendency — context for first-half bets</div>',unsafe_allow_html=True)
                    half_rows = []
                    for r2 in results:
                        hh = team_half[team_half["team"]==r2["Home"]]["1H%"].values
                        ah = team_half[team_half["team"]==r2["Away"]]["1H%"].values
                        half_rows.append({"Match":f"{r2['Home']} vs {r2['Away']}",
                            "Home 1H%":f"{hh[0]:.0f}%" if len(hh) else "—",
                            "Away 1H%":f"{ah[0]:.0f}%" if len(ah) else "—"})
                    st.dataframe(pd.DataFrame(half_rows), use_container_width=True, height=200)
            except: pass
        ...
        for r in results:
            ...
            with st.container():
                ...
                with st.expander("📝 Log a bet for this match"):
                    ...
                    bet_type=bl1.selectbox("Bet type",["Line","H2H","Total"],key=f"btype_{_ri}")
                    bet_side=bl2.text_input("Bet side",value=r["LineBet"] if bet_type=="Line" else r["H2HBet"],key=f"bside_{_ri}")
                    bet_odds=bl3.number_input("Odds",1.01,20.0,r["LineOdds"] if bet_type=="Line" else r["H2HOdds"],key=f"bodds_{_ri}",format="%.2f")
                    bet_stake=bl4.number_input("Stake ($)",0.0,10000.0,float(r["KellyLine"] if bet_type=="Line" else r["KellyH2H"]),key=f"bstake_{_ri}")
                    if st.button("Log bet",key=f"blog_{_ri}"):
                        add_bet({
...
with tabs[6]:
    ...
    if inp_mode=="CSV upload (from stats programme)":
        # ── Format selector ──────────────────────────────────────────────────
        csv_fmt = st.radio("CSV format", 
            ["Master format (A_/B_ columns)", "Gerard format (foxsports + match_stats)"],
            horizontal=True, key="csv_fmt")

        if csv_fmt == "Master format (A_/B_ columns)":
            st.markdown('<div class="info">Upload a CSV in master format: one row per match with A_ and B_ column prefixes.</div>',unsafe_allow_html=True)
            csv_file=st.file_uploader("Upload CSV",type=["csv"],key="csv_master")
            if csv_file:
                try:
                    new_data=pd.read_csv(csv_file)
                    if "Round" in new_data.columns:
                        csv_round=int(new_data["Round"].iloc[0])
                        if csv_round!=inp_rnd: inp_rnd=csv_round; st.info(f"Round {inp_rnd} detected")
            ...
            st.markdown('<div class="info">Upload the <b>two</b> Gerard CSV files:<br>1. <code>foxsports_nrl_YYYY_RoundNN_team_stats.csv</code><br>2. <code>nrl_YYYY_match_stats_rounds_NN.csv</code></div>',unsafe_allow_html=True)
            gc1,gc2=st.columns(2)
            gerard_fox  = gc1.file_uploader("Foxsports team stats CSV",type=["csv"],key="gerard_fox")
            gerard_match= gc2.file_uploader("Match stats CSV (scores)",type=["csv"],key="gerard_match")
            if gerard_fox and gerard_match:
                try:
                    fs_df = pd.read_csv(gerard_fox)
                    ms_df = pd.read_csv(gerard_match)
                    new_data = parse_gerard_round(fs_df, ms_df, season=int(season))
                    csv_round = int(new_data["Round"].iloc[0])
                    if csv_round != inp_rnd: inp_rnd = csv_round
                    st.success(f"Parsed {len(new_data)} matches — Round {inp_rnd}")
                    st.dataframe(new_data[["A Team","B Team","A_Points Scored","B_Points Scored",
                                          "A_Errors","B_Errors","A_Line Break","B_Line Break"]],
                                 use_container_width=True)
                except Exception as e: st.error(f"Gerard parse error: {e}"); new_data=None

            # Optional extended Gerard files
            st.markdown('<div class="shr">Optional — extended Gerard files</div>',unsafe_allow_html=True)
            ge1,ge2,ge3 = st.columns(3)
            gerard_player  = ge1.file_uploader("Player stats CSV",  type=["csv"], key="gerard_player")
            gerard_trysco  = ge2.file_uploader("Tryscorers CSV",     type=["csv"], key="gerard_trysco")
            gerard_restart = ge3.file_uploader("Set restarts CSV",   type=["csv"], key="gerard_restart")
            if gerard_player:
                try:
                    pdf = pd.read_csv(gerard_player)
                    # Accumulate: merge with existing
                    existing = st.session_state.get("gerard_player_df")
                    if existing is not None:
                        combined = pd.concat([existing, pdf]).drop_duplicates(subset=["round","team","player"])
                    else:
                        combined = pdf
                    st.session_state["gerard_player_df"] = combined
                    rounds_in = sorted(combined["round"].unique())
                    st.success(f"Player stats loaded: {len(combined)} rows, rounds {rounds_in}")
                except Exception as e: st.error(f"Player stats error: {e}")
            if gerard_trysco:
                try:
                    tdf = pd.read_csv(gerard_trysco)
                    existing = st.session_state.get("gerard_tryscore_df")
                    combined = pd.concat([existing, tdf]).drop_duplicates() if existing is not None else tdf
                    st.session_state["gerard_tryscore_df"] = combined
                    st.success(f"Tryscorers loaded: {len(combined)} rounds")
                except Exception as e: st.error(f"Tryscorers error: {e}")
            if gerard_restart:
                try:
                    rdf = pd.read_csv(gerard_restart)
                    existing = st.session_state.get("gerard_restart_df")
                    combined = pd.concat([existing, rdf]).drop_duplicates() if existing is not None else rdf
                    st.session_state["gerard_restart_df"] = combined
                    st.success(f"Set restarts loaded: {len(combined)} rows")
                except Exception as e: st.error(f"Set restarts error: {e}")
    else:
    ...
    if sub_btn and new_data is not None and len(new_data)>0:
        with st.spinner("Updating..."):
            # Always read fresh from Sheets (bypass cache) before running pipeline
            if has_sheets_config():
                fresh_df, ok = read_master_from_sheets(league)
                if ok and fresh_df is not None and len(fresh_df) > 0:
                    Rn = run_pipeline(None, league, target_season=int(season),
                                      new_matches_df=new_data, raw_df=fresh_df)
                else:
                    Rn = run_pipeline(str(master_path), league,
                                      target_season=int(season), new_matches_df=new_data)
            else:
                Rn = run_pipeline(str(master_path), league,
                                  target_season=int(season), new_matches_df=new_data)
        ...
        if has_sheets_config():
            ...
            if saved:
                st.success(f"Round {inp_rnd} saved to Google Sheets! Refreshing...")
                st.session_state["force_reload"] = True
                st.cache_data.clear()
                import time; time.sleep(1.5)  # Give Sheets a moment to commit
                st.rerun()
            else:
                st.warning("Could not save to Sheets — download the xlsx below.")
        else:
            st.success(f"Round {inp_rnd} added! Download Updated Master xlsx below.")
...
with tabs[7]:
    ...
    player_df = st.session_state.get("gerard_player_df")
    trysco_df = st.session_state.get("gerard_tryscore_df")
    
    if player_df is None:
        st.markdown('<div class="info">Upload player stats CSV in Weekly Input → Gerard format → "Player stats CSV" to unlock this tab.</div>', unsafe_allow_html=True)
    else:
        avail_rounds = sorted(player_df["round"].unique())
        teams_in = sorted(player_df["team"].unique())
        
        # ── Controls ──────────────────────────────────────────────────────────
        pc1, pc2, pc3 = st.columns([2,2,2])
        sel_stat = pc1.selectbox("Stat", list(PLAYER_DISPLAY.keys()),
                                  format_func=lambda x: PLAYER_DISPLAY.get(x,x), key="ps_stat")
        sel_round_filter = pc2.multiselect("Rounds", avail_rounds, 
                                            default=avail_rounds[-3:] if len(avail_rounds)>=3 else avail_rounds,
                                            key="ps_rounds")
        show_team = pc3.selectbox("Filter by team", ["All teams"] + teams_in, key="ps_team")
        
        if not sel_round_filter: sel_round_filter = avail_rounds
        
        # ── Top 20 players chart ───────────────────────────────────────────────
        st.markdown('<div class="shr">Top players — ' + PLAYER_DISPLAY.get(sel_stat,sel_stat) + '</div>', unsafe_allow_html=True)
        
        top_df = get_top_players(player_df, stat=sel_stat, n=20, rounds=sel_round_filter)
        if show_team != "All teams":
            top_df = top_df[top_df["team"]==show_team]
        
        if len(top_df):
            fig_top = go.Figure(go.Bar(
                y=top_df["player"] + " (" + top_df["team"].str.split().str[-1] + ")",
                x=top_df[sel_stat], orientation="h",
                marker_color="#3B82F6",
                text=top_df[sel_stat].round(0).astype(int).astype(str),
                textposition="outside", textfont=dict(color="#F1F5F9", size=10),
                hovertemplate="%{y}: %{x}<extra></extra>",
            ))
            at(fig_top, height=max(320, len(top_df)*32),
               xaxis_title=PLAYER_DISPLAY.get(sel_stat, sel_stat),
               title_text=f"Top 20 — {PLAYER_DISPLAY.get(sel_stat,sel_stat)} (R{min(sel_round_filter)}-R{max(sel_round_filter)})")
            st.plotly_chart(fig_top, use_container_width=True)
        
        # ── Form players (trending up) ─────────────────────────────────────────
        st.markdown('<div class="shr">Form players — trending up in last 3 rounds</div>', unsafe_allow_html=True)
        form_df = get_form_players(player_df, stat=sel_stat, window=3)
        if len(form_df):
            form_df = form_df[form_df["recent_avg"]>0].head(15)
            if show_team != "All teams":
                form_df = form_df[form_df["team"]==show_team]
            if len(form_df):
                fig_form = go.Figure(go.Bar(
                    y=form_df["player"] + " (" + form_df["team"].str.split().str[-1] + ")",
                    x=form_df["trend"], orientation="h",
                    marker_color=["#10B981" if v>=0 else "#EF4444" for v in form_df["trend"]],
                    text=[f"{v:+.0f}" for v in form_df["trend"]],
                    textposition="outside", textfont=dict(color="#F1F5F9", size=10),
                    hovertemplate="%{y}: %{x:+.1f} trend<extra></extra>",
                ))
                fig_form.add_vline(x=0, line_dash="dash", line_color="#64748B")
                at(fig_form, height=max(300, len(form_df)*32),
                   xaxis_title=f"Trend (last 3 rounds vs before)",
                   title_text="Form players — rising vs previous form")
                st.plotly_chart(fig_form, use_container_width=True)
        
        # ── 1H vs 2H breakdown ────────────────────────────────────────────────
        if trysco_df is not None:
            st.markdown('<div class="shr">First half vs Second half — try scoring</div>', unsafe_allow_html=True)
            half_df = build_halfline_table(trysco_df)
            if len(half_df):
                # Aggregate by team across rounds
                team_half = half_df.groupby("team")[["1H","2H","total"]].sum().reset_index()
                team_half["1H%"] = (team_half["1H"] / team_half["total"] * 100).round(1)
                team_half = team_half.sort_values("1H%", ascending=True)
                
                fig_half = go.Figure()
                fig_half.add_trace(go.Bar(
                    y=team_half["team"], x=team_half["1H%"], name="1H tries %",
                    orientation="h", marker_color="#3B82F6", opacity=0.85,
                    hovertemplate="%{y}: %{x:.1f}% in 1H<extra></extra>",
                ))
                fig_half.add_vline(x=50, line_dash="dash", line_color="#64748B",
                                    annotation_text="50%")
                at(fig_half, height=max(300, len(team_half)*30),
                   xaxis=dict(title="% of tries scored in 1st half", tickformat=".0f",
                               range=[0,100], gridcolor="#334155"),
                   title_text="1H scoring tendency — above 50% = fast starters")
                st.plotly_chart(fig_half, use_container_width=True)
                
                st.markdown('<div class="info">Teams scoring >50% of tries in 1H are "fast starters" — useful for 1H line bets.</div>', unsafe_allow_html=True)
        
        # ── Team summary table ────────────────────────────────────────────────
        st.markdown('<div class="shr">Team aggregates — selected rounds</div>', unsafe_allow_html=True)
        team_agg = aggregate_player_to_team(player_df[player_df["round"].isin(sel_round_filter)])
        disp_stats = ["all_run_metres","p_c_m","line_breaks","tackle_breaks",
                       "tackles","missed_tackles","errors","tries"]
        disp_stats = [s for s in disp_stats if s in team_agg.columns]
        team_sum = team_agg.groupby("team")[disp_stats].sum().reset_index()
        team_sum = team_sum.rename(columns={s:PLAYER_DISPLAY.get(s,s) for s in disp_stats})
        if show_team != "All teams":
            team_sum = team_sum[team_sum["team"]==show_team]
        st.dataframe(team_sum.sort_values("Run Metres" if "Run Metres" in team_sum.columns else team_sum.columns[1], 
                                           ascending=False),
                     use_container_width=True, height=400)
        st.download_button("Download CSV", 
                            team_sum.to_csv(index=False).encode(),
                            f"{league}_{season}_player_team_agg.csv", "text/csv")
