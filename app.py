...
if not master_path.exists():
    st.error(f"Upload `{master_path.name}` to the repo first."); st.stop()
...
try: R=get_result(str(master_path),league,int(season))
...
A_Points Scored, B_Points Scored</code> + stat columns (A_StatName, B_StatName)
    ...
    if sub_btn and new_data is not None and len(new_data)>0:
        ...
        # Auto-save updated master to disk so next reload picks it up
        upd_auto = Rn["df"]
        keep_auto = [c for c in upd_auto.columns if not c.startswith("Diff_Form")
                     and not c.startswith("A_Form") and not c.startswith("B_Form")]
        upd_auto[keep_auto].to_excel(str(master_path), index=False)
        st.cache_data.clear()  # Clear again so dashboard reloads from new file
        st.success(f"Round {inp_rnd} added & saved! All outputs updated.")
